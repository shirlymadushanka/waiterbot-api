...
describe('POST /api/auth/client_register', () => {
    ...
    it('first_name required case passed.', (done) => {
        request(app).post('/api/auth/client_register')
          .send({ last_name: 'fsdfsfsdf', password : "@fjsklfD2", mobile : "+94717726160" })
          .then((res) => {
            const body = res.body;
    ...
    it('password required case passed.', (done) => {
        request(app).post('/api/auth/client_register')
          .send({ first_name: 'sfdsfsdf', last_name: 'fsdfsfsdf', mobile : "+94717726160" })
          .then((res) => {
            const body = res.body;
            expect(body.success).to.equal(false);
            done();
    ...
    it('Client registration passed.', (done) => {
        request(app).post('/api/auth/client_register')
          .send({ first_name: 'sfdsfsdf', last_name: 'fsdfsfsdf', password : "@fjsklfD2", mobile : "+94717726160" })
          .then((res) => {
...
describe('POST /api/auth/client_login', () => {
  ...
  it('Client login passed.', (done) => {
      request(app).post('/api/auth/client_login').send({ password : "@fjsklfD2", mobile : "+94717726160" })
      .then((res) => {
          expect(res.body.success).to.equal(true);
