...
describe('POST /api/auth/admin_register', () => {
  ...
  it('first_name required case passed.', (done) => {
    request(app).post('/api/auth/admin_register')
      .send({ last_name: 'fsdfsfsdf', password: "@fjsklfD2", mobile: "0717726169" })
      .then((res) => {
        const body = res.body;
  ...
  it('password required case passed.', (done) => {
    request(app).post('/api/auth/admin_register')
      .send({ first_name: 'sfdsfsdf', last_name: 'fsdfsfsdf', mobile: "0717726169" })
      .then((res) => {
        const body = res.body;
        expect(body.success).to.equal(false);
        done();
  ...
  it('Admin registration passed.', (done) => {
    request(app).post('/api/auth/admin_register')
      .send({ first_name: 'sfdsfsdf', last_name: 'fsdfsfsdf', password: "@fjsklfD2", mobile: "0717726169" })
      .then((res) => {
...
describe('POST /api/auth/admin_login', () => {
  ...
  it('Admin login passed.', (done) => {
    request(app).post('/api/auth/admin_login').send({ password: "@fjsklfD2", mobile: "0717726169" })
      .then((res) => {
        expect(res.body.success).to.equal(true);
